...
class RoutesRequest extends FormRequest
{
    ...
    public function prepareForValidation()
    {
        ...
        $map = collect($this->detail)->map(function ($item){
            $data[Menus::field_primary()] = $item['temp_id'];
    ...
    public function withValidator($validator)
    {
        $validator->after(function ($validator) {
            ...
            $check = Routes::where(Routes::field_primary(), $this->route_slug)->get()->count();
            if($check){
                $validator->errors()->add(Routes::field_primary(), 'Code must unique!');
            }
